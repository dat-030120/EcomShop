...
export default function Dangky() {
  const erro = useSelector((state) => state.user?.error);
...
const handle =()=>{
  ...
  const handleUpload = (values) => {
    if(file ===null){
      const product = {username:values.username,email:values.email,password:values.password, }
      regis(dispatch,product).then((result)=>{
        history(result)})
    }
    else
      ...
      uploadTask.on(
        ...
        () => {  
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            const product = { username:values.username,email:values.email,password:values.password , img: downloadURL, };
  ...
  return (
    <div className="all"><div className="newProduct">
    <h1 className="addProductTitle">New User</h1>
      <form className="addProduct"  onSubmit={formik.handleSubmit}>
        ...
        <button  onClick={ handle} className="addProductButton">
        Login
        </button>
      </form>
    </div></div>
